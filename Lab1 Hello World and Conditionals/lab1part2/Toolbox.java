...
public class Toolbox {
	...
	public Integer getRandomInteger(Integer max) {
		...
		if (Toolbox.testing) {
			...
			if (number == null) {
				throw new RuntimeException("Ran out of random integers from the test data. This may indicate that getRandomInteger was called too many times.");
			}

			int value = number.intValue();
			if (value < 1 || value > max) {
				throw new RuntimeException("Integer from test data was outside the expected range. This may indicate that getRandomInteger was called with an inappropriate range.");
			}
	...
	public Integer readIntegerFromCmd() {
		...
		try {
			...
			if (Toolbox.testing) {
				throw new RuntimeException("Test input was not an integer. This may indicate that readIntegerFromCmd or readStringFromCmd was called the wrong number of times.");
			} else {
	...
	private String readStringInternal() {
		if (Toolbox.testing) {
			...
			if (testInput == null) {
				throw new RuntimeException("Ran out of random inputs from the test data. This may indicate that readIntegerFromCmd or readStringFromCmd was called too many times.");
			}
